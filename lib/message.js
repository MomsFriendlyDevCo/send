export default class SendMessage {
	...
	* Each key is the source format, a hyphen + higher-than and the destination format with optional whitespace
	...
	*/
	as(format) {
		...
		Object.entries(this.converters)
			.some(([key, converter]) => { // Loop until we find a converter
				let {from, to} = /(?<from>.+?)\s*->\s*(?<to>.+)$/.exec(key)?.groups || {};
				if (to == format && this.content[from]) { // We have a valid conversion path
					this.set(to, converter(this.content[from]));
					return true; // Exit looping over converters
				}
			})

		// Did we convert formats from above?
		if (this.content[format]) return Promise.resolve(this.content[format]);

		throw new Error(`Cannot provide or convert to message format "${format}" from available formats: ` + Object.keys(this.content).join(', '));
	}
